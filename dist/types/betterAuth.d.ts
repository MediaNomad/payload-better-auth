 ...
 */
export type RoleArray<O extends readonly string[] = readonly ['user']> = O[number][] | null;
/**
 ...
 * ```
 */
export type PayloadWithAuth<O extends BetterAuthOptions = BetterAuthOptions> = BasePayload & {
 ...
 * ```
 */
export interface PayloadRequestWithBetterAuth<O extends BetterAuthOptions = BetterAuthOptions> extends PayloadRequest {
 ...
 * ```
 */
export type CollectionHookWithBetterAuth<O extends BetterAuthOptions, T extends (args: Record<string, unknown>) => unknown> = T extends (args: infer A) => infer R ? (args: Omit<A, 'req'> & {
